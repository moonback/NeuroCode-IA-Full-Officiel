            ...
            if (startingIdx > 0) {
              ...
              filteredMessages = [
                ...
                },
                {
                  ...
                  role: 'assistant',
                  content: `📦 Projet restauré avec succès à partir de la sauvegarde instantanée
                  <boltArtifact id="imported-files" title="Fichiers du projet restaurés" type="bundled">
...
function navigateChat(nextId: string) {
  // Debounce navigation to prevent IPC flooding
  let lastNavigation = 0;
  const now = Date.now();
  const DEBOUNCE_TIME = 100; // 100ms debounce time

  if (now - lastNavigation < DEBOUNCE_TIME) {
    return;
  }
