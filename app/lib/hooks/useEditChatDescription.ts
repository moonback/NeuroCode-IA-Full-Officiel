...
export function useEditChatDescription({
  ...
  const isValidDescription = useCallback((desc: string): boolean => {
    ...
    if (!lengthValid) {
      toast('Description must be between 1 and 100 characters.');
      return false;
    }

    if (!characterValid) {
      toast('Description can only contain letters, numbers, spaces, and basic punctuation.');
      return false;
    }
  ...
  const handleSubmit = useCallback(
    async (event: React.FormEvent) => {
      ...
      try {
        if (!db) {
          toast('Chat persistence is not available');
          return;
        }

        if (!chatId) {
          toast('Chat Id is not available');
          return;
        }
        ...
        toast('Chat description updated successfully');
      } catch (error) {
        toast('Failed to update chat description: ' + (error as Error).message);
      }
