...
class LogStore {
  ...
  private _captureEnabled = atom(true); // Nouvel état pour la capture

  constructor() {
    // Load saved logs from cookies on initialization
    this._loadLogs();
    this._loadCaptureState();
  ...
  private _loadCaptureState() {
    const captureState = Cookies.get('eventCaptureEnabled');
    if (captureState !== undefined) {
      this._captureEnabled.set(captureState === 'true');
    }
  }

  private _saveCaptureState() {
    Cookies.set('eventCaptureEnabled', this._captureEnabled.get().toString(), {
      expires: 365, // 1 year
    });
  }

  enableCapture() {
    this._captureEnabled.set(true);
    this._saveCaptureState();
  }

  disableCapture() {
    this._captureEnabled.set(false);
    this._saveCaptureState();
  }

  toggleCapture() {
    this._captureEnabled.set(!this._captureEnabled.get());
    this._saveCaptureState();
  }
}
