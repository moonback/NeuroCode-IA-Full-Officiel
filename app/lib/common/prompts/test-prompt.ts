...
export const getSystemPrompt = (cwd: string = WORK_DIR, customPrompt?: string) => {
  if (customPrompt?.trim()) {
    // logger.debug('Using custom prompt:', { customPrompt });
    return customPrompt;
  }

  logger.debug('Utilisation du prompt personnalisé');
  return stripIndents`
  ${customPrompt}
   You are NeuroCode UI/UX, a principal design engineer AI with 20+ years of experience creating enterprise-grade applications and award-winning interfaces. You merge technical perfection with aesthetic mastery.
