...
const TAB_ICONS: Record<TabType, string> = {
  ...
  'custom-prompt': 'i-ph:chat-text-fill',
};
...
const OPTIONAL_USER_TABS: TabType[] = ['settings', 'task-manager', 'service-status', 'debug', 'update', 'custom-prompt'];

// All available tabs for user mode
const ALL_USER_TABS = [...DEFAULT_USER_TABS, ...OPTIONAL_USER_TABS];

// Define which tabs are beta
const BETA_TABS = new Set<TabType>(['task-manager', 'service-status', 'update', 'local-providers', 'custom-prompt']);

// Beta label component
const BetaLabel = () => (
  <span className="px-1.5 py-0.5 text-[10px] text-white rounded-full bg-green-500/10 text-green-500 font-medium">BETA</span>
);
...
export const TabManagement = () => {
  ...
  const handleTabVisibilityChange = (tabId: TabType, checked: boolean) => {
    ...
    if (!currentTab) {
      ...
      toast(`Onglet ${checked ? 'activé' : 'désactivé'} avec succès`);
    ...
    if (!canBeEnabled && checked) {
      toast('Cet onglet ne peut pas être activé en mode utilisateur');
      return;
    }
      ...
      >
        ...
        <div className="flex items-center justify-between gap-4 mt-8 mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h4 className="text-md font-medium text-bolt-elements-textPrimary">Gestion des onglets</h4>
              <p className="text-sm text-bolt-elements-textSecondary">Configurez les onglets visibles et leur ordre</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {filteredTabs.some((tab) => DEFAULT_USER_TABS.includes(tab.id)) && (
            <div className="col-span-full flex items-center gap-2 mt-4 mb-2">
              ...
              <span className="text-sm font-medium text-bolt-elements-textPrimary">Onglets par défaut</span>
            </div>
          )}
            ...
            .map((tab, index) => (
              ...
              >
                ...
                <div className="absolute top-1 right-1.5 flex gap-1">
                  <span className="px-1.5 py-0.25 text-white text-xs rounded-full bg-green-500/50 text-green-500 font-medium mr-2">
                    Default
                  </span>
                </div>
          ...
          {filteredTabs.some((tab) => OPTIONAL_USER_TABS.includes(tab.id)) && (
            <div className="col-span-full flex items-center gap-2 mt-8 mb-2">
              ...
              <span className="text-sm font-medium text-bolt-elements-textPrimary">Onglets optionnels</span>
            </div>
          )}
          ...
          {filteredTabs
            ...
            .map((tab, index) => (
              ...
              >
                {/* Status Badges */}
                <div className="absolute top-1 right-1.5 flex gap-1">
                  <span className="px-1.5 py-0.25 text-white text-xs rounded-full bg-blue-500/50 text-blue-500 font-medium mr-2">
                    Optional
                  </span>
                </div>
