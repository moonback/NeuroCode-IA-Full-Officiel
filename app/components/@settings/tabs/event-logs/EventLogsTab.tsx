    ...
    if (log.category === 'provider') {
      return (
        <div className="flex flex-col gap-2">
          ...
          {details.response && (
            <div className="flex flex-col gap-1">
              <div className="text-xs font-medium text-gray-700 dark:text-gray-300">Reponse:</div>
              <pre className="text-xs text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/50 rounded p-2 whitespace-pre-wrap">
                {details.response}
              </pre>
            </div>
    ...
    if (log.category === 'api') {
      return (
        <div className="flex flex-col gap-2">
          ...
          {details.request && (
            <div className="flex flex-col gap-1">
              <div className="text-xs font-medium text-gray-700 dark:text-gray-300">Requête:</div>
              <pre className="text-xs text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/50 rounded p-2 whitespace-pre-wrap">
          ...
          {details.response && (
            <div className="flex flex-col gap-1">
              <div className="text-xs font-medium text-gray-700 dark:text-gray-300">Réponse:</div>
              <pre className="text-xs text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/50 rounded p-2 whitespace-pre-wrap">
          ...
          {details.error && (
            <div className="flex flex-col gap-1">
              <div className="text-xs font-medium text-red-500">Erreur:</div>
            ...
            {log.details && (
              <>
                <button
                  ...
                  className="text-xs bg-emerald-500 text-black dark hover:text-black-500 dark:hover:text-black-400 transition-colors"
                >
                  {localExpanded ? 'Masquer' : 'Afficher'} les détails
                </button>
...
export function EventLogsTab() {
  ...
  const captureEnabled = useStore(logStore.captureEnabled);
  ...
  useEffect(() => {
    ...
    logStore.logInfo('Onglet Journaux d\'événements monté', {
      type: 'component_mount',
      message: 'Onglet Journaux d\'événements monté',
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (searchQuery) {
        logStore.logInfo('Recherche de logs effectuée', {
          type: 'search',
          message: `Recherche effectuée avec la requête "${searchQuery}" (${filteredLogs.length} résultats)`,
          component: 'EventLogsTab',
          query: searchQuery,
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      ...
      logStore.logSuccess('Journaux rafraîchis avec succès', {
        type: 'refresh',
        message: `Journaux rafraîchis avec succès (${Object.keys(logs).length} journaux)`,
        component: 'EventLogsTab',
        duration,
        logsCount: Object.keys(logs).length,
      });
    } catch (error) {
      logStore.logError('Échec du rafraîchissement des journaux', error, {
        type: 'refresh_error',
        message: 'Échec du rafraîchissement des journaux',
        component: 'EventLogsTab',
      });
    } finally {
  ...
  const handlePreferenceChange = useCallback((type: string, value: boolean) => {
    logStore.logInfo('Log preference changed', {
      ...
      message: `Préférence de journal "${type}" changée en ${value}`,
      component: 'EventLogsTab',
      preference: type,
      value,
    });
  ...
  const exportAsJSON = () => {
    try {
      ...
      toast('Journaux exportés avec succès en JSON');
    } catch (error) {
      console.error('Échec de l\'exportation en JSON:', error);
      toast('Échec de l\'exportation des journaux en JSON');
    }
  };
  ...
  const exportAsCSV = () => {
    try {
      ...
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
      toast('Journaux exportés avec succès en CSV');
    } catch (error) {
      console.error('Échec de l\'exportation en CSV:', error);
      toast('Échec de l\'exportation des journaux en CSV');
    }
  };
  ...
  const exportAsPDF = () => {
    try {
      ...
      toast('Journaux exportés avec succès en PDF');
    } catch (error) {
      console.error('Échec de l\'exportation en PDF:', error);
      toast('Échec de l\'exportation des journaux en PDF');
    }
  };
  ...
  const exportAsText = () => {
    try {
      ...
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
      toast('Journaux exportés avec succès en texte');
    } catch (error) {
      console.error('Échec de l\'exportation en texte:', error);
      toast('Échec de l\'exportation des journaux en texte');
    }
  };
  ...
  const handleClearAll = useCallback(async () => {
    try {
      ...
      toast.success('Tous les journaux d\'événements ont été effacés avec succès');
    } catch (error) {
      console.error('Échec de l\'effacement des journaux:', error);
      toast('Échec de l\'effacement des journaux');
    }
  }, []);

  const handleToggleCapture = useCallback(() => {
    logStore.toggleCapture();
    toast.success(`Capture d'événements ${logStore.captureEnabled.get() ? 'activée' : 'désactivée'}`, {
      icon: logStore.captureEnabled.get() ? '✅' : '❌' as any,
    });
  }, []);
    ...
    <div className="flex h-full flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            />
            <span className="text-sm text-gray-500 dark:text-gray-400">Afficher les horodatages</span>
          </div>
          ...
          >
            <span
              className={classNames(
                'i-ph:record text-lg transition-colors',
                captureEnabled ? 'text-green-500' : 'text-red-500'
              )}
            />
            {captureEnabled ? 'Activée' : 'Désactivée'}
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4">
        <div className="relative">
          <input
            type="text"
            placeholder="Rechercher des journaux..."
            value={searchQuery}
        ...
        {filteredLogs.length === 0 ? (
          ...
          >
            ...
            <div className="flex flex-col gap-1">
              <h3 className="text-sm font-medium text-gray-900 dark:text-white">Aucun journal trouvé</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400">Essayez d'ajuster votre recherche ou vos filtres</p>
            </div>
          </motion.div>
        ) : (
          filteredLogs.map((log) => (
