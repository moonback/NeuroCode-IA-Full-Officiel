...
export function PushToGitHubDialog({ isOpen, onClose, onPush }: PushToGitHubDialogProps) {
  ...
  const fetchRecentRepos = async (token: string) => {
    if (!token) {
      logStore.logError('Aucun jeton GitHub disponible');
      toast('Authentification GitHub requise');
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          toast('Jeton GitHub expiré. Veuillez vous reconnecter.');
          ...
          logStore.logError('Échec de la récupération des dépôts GitHub', {
            status: response.status,
            statusText: response.statusText,
            error: errorData,
          });
          toast(`Échec de la récupération des dépôts GitHub: ${response.statusText}`);
        }
      ...
      logStore.logError('Échec de la récupération des dépôts GitHub', { error });
      toast('Échec de la récupération des dépôts GitHub');
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!connection?.token || !connection?.user) {
      toast('Veuillez vous connecter à votre compte GitHub dans Paramètres > Connexions d\'abord');
      return;
    }

    if (!repoName.trim()) {
      toast('Le nom du dépôt est requis');
      return;
    }
    ...
    try {
      ...
      try {
        ...
        const confirmOverwrite = window.confirm(
          `Le dépôt "${repoName}" existe déjà. Voulez-vous le mettre à jour ? Cela ajoutera ou modifiera les fichiers dans le dépôt.`,
        );
      ...
      toast('Échec de la publication sur GitHub. Veuillez vérifier le nom du dépôt et réessayer.');
    } finally {
      setIsLoading(false);
    }
  };
                ...
                <div className="p-6 space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 text-green-500">
                      ...
                      <h3 className="text-lg font-medium">Publié avec succès sur GitHub</h3>
                    </div>
                    <Dialog.Close
                      onClick={handleClose}
                  ...
                  <div className="bg-bolt-elements-background-depth-2 dark:bg-bolt-elements-background-depth-3 rounded-lg p-3">
                    <p className="text-xs text-bolt-elements-textSecondary dark:text-bolt-elements-textSecondary-dark mb-2">
                      Fichiers publiés ({pushedFiles.length})
                    </p>
              ...
              <Dialog.Content className="bg-white dark:bg-[#0A0A0A] rounded-lg p-6 border border-[#E5E5E5] dark:border-[#1A1A1A] shadow-xl">
                <div className="text-center space-y-4">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    Veuillez vous connecter à votre compte GitHub dans Paramètres {'>'} Connexions pour publier votre code sur GitHub.
                  </p>
                  <motion.button
              ...
              <div className="p-6">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      Publiez votre code sur un nouveau ou un dépôt GitHub existant
                    </p>
                  </div>
                  <Dialog.Close
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {recentRepos.length > 0 && (
                    <div className="space-y-2">
                      <label className="text-sm text-gray-600 dark:text-gray-400">Dépôts récents</label>
                      <div className="space-y-2">
