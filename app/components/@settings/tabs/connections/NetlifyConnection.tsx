...
export function NetlifyConnection() {
  ...
  const handleConnect = async (event: React.FormEvent) => {
    ...
    try {
      ...
      await fetchNetlifyStats(connection.token);
      toast.success('Connexion réussie à Netlify');
    } catch (error) {
      console.error('Erreur d\'authentification:', error);
      logStore.logError('Échec de l\'authentification avec Netlify', { error });
      toast.error('Échec de la connexion à Netlify');
  ...
  const handleDisconnect = () => {
    updateNetlifyConnection({ user: null, token: '' });
    toast.success('Déconnecté de Netlify');
  };
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            />
            <h3 className="text-base font-medium text-bolt-elements-textPrimary">Connexion Netlify</h3>
          </div>
        </div>
        ...
        {!connection.user ? (
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-bolt-elements-textSecondary mb-2">Token personnel</label>
              <input
                type="password"
                value={connection.token}
                onChange={(e) => updateNetlifyConnection({ ...connection, token: e.target.value })}
                disabled={connecting}
                placeholder="Entrez votre token personnel Netlify"
          ...
          <div className="space-y-6">
            ...
            {fetchingStats ? (
              <div className="flex items-center gap-2 text-sm text-bolt-elements-textSecondary">
                ...
                Récupération des sites Netlify...
              </div>
            ) : (
              <div>
                <button
                ...
                >
                  ...
                  Vos sites ({connection.stats?.totalSites || 0})
                  <div
                    className={classNames(
                ...
                {isSitesExpanded && connection.stats?.sites?.length ? (
                  ...
                  <div className="text-sm text-bolt-elements-textSecondary flex items-center gap-2">
                    ...
                    Aucun site trouvé dans votre compte Netlify
                  </div>
                ) : null}
              </div>
            )}
