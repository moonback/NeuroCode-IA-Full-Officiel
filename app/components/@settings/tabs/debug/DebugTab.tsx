      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="text-base text-bolt-elements-textPrimary">
            {title} Dépendances ({deps.length})
          </span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-sm text-bolt-elements-textSecondary">{isOpen ? 'Masquer' : 'Afficher'}</span>
          <div
            className={classNames(
...
export default function DebugTab() {
  ...
  useEffect(() => {
    ...
    const handleRejection = (event: PromiseRejectionEvent) => {
      logStore.logError('Rejet de promesse non géré', event.reason);
    };
  ...
  useEffect(() => {
    ...
    const fetchGitInfo = async () => {
      try {
        ...
        console.error('Impossible de récupérer les informations git:', error);
      }
    };
  ...
  const getSystemInfo = async () => {
    try {
      ...
      toast('Informations du système mises à jour');
    } catch (error) {
      toast('Impossible de récupérer les informations du système');
      console.error('Impossible de récupérer les informations du système:', error);
    } finally {
  ...
  const getWebAppInfo = async () => {
    try {
      ...
      toast('Informations de l\'application mises à jour');

      return true;
    } catch (error) {
      console.error('Impossible de récupérer les informations de l\'application:', error);
      toast('Impossible de récupérer les informations de l\'application');
      setWebAppInfo(null);
  ...
  const handleLogPerformance = () => {
    try {
      ...
      toast('Métriques de performance enregistrées');
    } catch (error) {
      toast('Impossible d\'enregistrer les métriques de performance');
      console.error('Impossible d\'enregistrer les métriques de performance:', error);
    } finally {
  ...
  const checkErrors = async () => {
    try {
      ...
      if (storedErrors.length === 0) {
        toast('Aucune erreur trouvée');
      } else {
        toast.warning(`${storedErrors.length} erreur(s) trouvée(s)`);
      }
    } catch (error) {
      toast('Impossible de vérifier les erreurs');
      console.error('Impossible de vérifier les erreurs:', error);
    } finally {
  ...
  const exportDebugInfo = () => {
    try {
      ...
      toast('Informations de débogage exportées avec succès');
    } catch (error) {
      console.error('Impossible d\'exporter les informations de débogage:', error);
      toast('Impossible d\'exporter les informations de débogage');
    }
  };
  ...
  const exportAsCSV = () => {
    try {
      ...
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
      toast('Informations de débogage exportées en CSV');
    } catch (error) {
      console.error('Impossible d\'exporter en CSV:', error);
      toast('Impossible d\'exporter les informations de débogage en CSV');
    }
  };
  ...
  const exportAsPDF = () => {
    try {
      ...
      doc.setFontSize(24);
      doc.setFont('helvetica', 'bold');
      doc.text('Rapport d\'informations de débogage', margin, 25);
      yPos = 50;
      ...
      toast('Informations de débogage exportées en PDF');
    } catch (error) {
      console.error('Impossible d\'exporter en PDF:', error);
      toast('Impossible d\'exporter les informations de débogage en PDF');
    }
  };
  ...
  const exportAsText = () => {
    try {
      ...
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
      toast('Informations de débogage exportées en fichier texte');
    } catch (error) {
      console.error('Impossible d\'exporter le fichier texte:', error);
      toast('Impossible d\'exporter les informations de débogage en fichier texte');
    }
  };
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-xs text-bolt-elements-textSecondary mt-0.5">
                      {format.id === 'json' && 'Exporter en fichier JSON structuré'}
                      {format.id === 'csv' && 'Exporter en fichier CSV'}
                      {format.id === 'pdf' && 'Exporter en document PDF formaté'}
                      {format.id === 'txt' && 'Exporter en fichier texte formaté'}
                    </div>
                  </div>
                </button>
              ))}
  ...
  const getOllamaStatus = () => {
    ...
    if (!isOllamaEnabled) {
      return {
        ...
        color: 'text-red-500',
        bgColor: 'bg-red-500',
        message: 'Le fournisseur Ollama est désactivé dans les paramètres',
      };
    }

    if (!ollamaStatus.isRunning) {
      return {
        status: 'Non en cours d\'exécution',
        color: 'text-red-500',
        bgColor: 'bg-red-500',
        message: ollamaStatus.error || 'Le service Ollama n\'est pas en cours d\'exécution',
      };
    }
    ...
    return {
      ...
      message: `Le service Ollama est en cours d'exécution avec ${modelCount} modèles installés (Fournisseur: Activé)`,
    };
  };
    ...
    <div className="flex flex-col gap-6 max-w-7xl mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="p-4 rounded-xl bg-white dark:bg-[#0A0A0A] border border-[#E5E5E5] dark:border-[#1A1A1A] hover:border-green-500/30 transition-all duration-200 h-[180px] flex flex-col">
          ...
          <div className="text-xs text-bolt-elements-textSecondary mt-2 flex items-center gap-1.5">
            ...
            />
            {errorLogs.length > 0 ? 'Erreurs détectées' : 'Aucune erreur détectée'}
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-white dark:bg-[#0A0A0A] border border-[#E5E5E5] dark:border-[#1A1A1A] hover:border-green-500/30 transition-all duration-200 h-[180px] flex flex-col">
          <div className="flex items-center gap-2">
            ...
            <div className="text-sm text-bolt-elements-textSecondary">Utilisation de la mémoire</div>
          </div>
          <div className="flex items-center gap-2 mt-2">
            <span
              className={classNames(
        ...
        <div className="p-4 rounded-xl bg-white dark:bg-[#0A0A0A] border border-[#E5E5E5] dark:border-[#1A1A1A] hover:border-green-500/30 transition-all duration-200 h-[180px] flex flex-col">
          <div className="flex items-center gap-2">
            ...
            <div className="text-sm text-bolt-elements-textSecondary">Temps de chargement de la page</div>
          </div>
          <div className="flex items-center gap-2 mt-2">
            <span
              className={classNames(
        ...
        <div className="p-4 rounded-xl bg-white dark:bg-[#0A0A0A] border border-[#E5E5E5] dark:border-[#1A1A1A] hover:border-green-500/30 transition-all duration-200 h-[180px] flex flex-col">
          <div className="flex items-center gap-2">
            ...
            <div className="text-sm text-bolt-elements-textSecondary">Vitesse du réseau</div>
          </div>
          <div className="flex items-center gap-2 mt-2">
            <span
              className={classNames(
      ...
      <div className="flex flex-wrap gap-4">
        ...
        >
          ...
          Récupérer les informations de l'application web
        </button>
        ...
        <CollapsibleTrigger className="w-full">
          <div className="flex items-center justify-between p-6 rounded-xl bg-white dark:bg-[#0A0A0A] border border-[#E5E5E5] dark:border-[#1A1A1A]">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-base font-medium text-bolt-elements-textPrimary">Informations du système</h3>
            </div>
            <div
              className={classNames(
        ...
        <CollapsibleContent>
          <div className="p-6 mt-2 rounded-xl bg-white dark:bg-[#0A0A0A] border border-[#E5E5E5] dark:border-[#1A1A1A]">
            {systemInfo ? (
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <div className="text-sm flex items-center gap-2">
                    ...
                    <span className="text-bolt-elements-textSecondary">Plateforme: </span>
                  ...
                  <div className="text-sm flex items-center gap-2">
                    ...
                    <span className="text-bolt-elements-textSecondary">Cœurs CPU: </span>
                  ...
                  <div className="text-sm flex items-center gap-2">
                    ...
                    <span className="text-bolt-elements-textSecondary">Version de Node: </span>
                  ...
                  <div className="text-sm flex items-center gap-2">
                    ...
                    <span className="text-bolt-elements-textSecondary">Type de réseau: </span>
                  ...
                  <div className="text-sm flex items-center gap-2">
                    ...
                    <span className="text-bolt-elements-textSecondary">Vitesse du réseau: </span>
                    <span className="text-bolt-elements-textPrimary">
                  ...
                  {systemInfo.battery && (
                    <div className="text-sm flex items-center gap-2">
                      ...
                      <span className="text-bolt-elements-textSecondary">Batterie: </span>
                      <span className="text-bolt-elements-textPrimary">
                  ...
                  <div className="text-sm flex items-center gap-2">
                    ...
                    <span className="text-bolt-elements-textSecondary">Stockage: </span>
                    <span className="text-bolt-elements-textPrimary">
                ...
                <div className="space-y-2">
                  <div className="text-sm flex items-center gap-2">
                    ...
                    <span className="text-bolt-elements-textSecondary">Utilisation de la mémoire: </span>
                    <span className="text-bolt-elements-textPrimary">
                  ...
                  <div className="text-sm flex items-center gap-2">
                    ...
                    <span className="text-bolt-elements-textSecondary">Navigateur: </span>
                    <span className="text-bolt-elements-textPrimary">
                  ...
                  <div className="text-sm flex items-center gap-2">
                    ...
                    <span className="text-bolt-elements-textSecondary">Écran: </span>
                    <span className="text-bolt-elements-textPrimary">
                  ...
                  <div className="text-sm flex items-center gap-2">
                    ...
                    <span className="text-bolt-elements-textSecondary">Fuseau horaire: </span>
                  ...
                  <div className="text-sm flex items-center gap-2">
                    ...
                    <span className="text-bolt-elements-textSecondary">Langue: </span>
                  ...
                  <div className="text-sm flex items-center gap-2">
                    ...
                    <span className="text-bolt-elements-textSecondary">Temps de chargement de la page: </span>
                    <span className="text-bolt-elements-textPrimary">
                  ...
                  <div className="text-sm flex items-center gap-2">
                    ...
                    <span className="text-bolt-elements-textSecondary">DOM prêt: </span>
                    <span className="text-bolt-elements-textPrimary">
              ...
              <div className="text-sm text-bolt-elements-textSecondary">Chargement des informations du système...</div>
            )}
          </div>
        </CollapsibleContent>
      </Collapsible>
        ...
        <CollapsibleTrigger className="w-full">
          <div className="flex items-center justify-between p-6 rounded-xl bg-white dark:bg-[#0A0A0A] border border-[#E5E5E5] dark:border-[#1A1A1A]">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-base font-medium text-bolt-elements-textPrimary">Métriques de performance</h3>
            </div>
            <div
              className={classNames(
                'i-ph:caret-down w-4 h-4 transform transition-transform duration-200',
            ...
            {systemInfo && (
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <div className="text-sm">
                    <span className="text-bolt-elements-textSecondary">Temps de chargement du DOM: </span>
                    <span className="text-bolt-elements-textPrimary">
                      {(systemInfo.performance.timing.domReadyTime / 1000).toFixed(2)}s
                    </span>
                  </div>
                  <div className="text-sm">
                    <span className="text-bolt-elements-textSecondary">Temps de requête: </span>
                    <span className="text-bolt-elements-textPrimary">
                      {(systemInfo.performance.timing.requestTime / 1000).toFixed(2)}s
                    </span>
                  </div>
                  <div className="text-sm">
                    <span className="text-bolt-elements-textSecondary">Temps de redirection: </span>
                    <span className="text-bolt-elements-textPrimary">
                ...
                <div className="space-y-2">
                  <div className="text-sm">
                    <span className="text-bolt-elements-textSecondary">Utilisation du JS Heap: </span>
                    <span className="text-bolt-elements-textPrimary">
                      {(systemInfo.performance.memory.usedJSHeapSize / (1024 * 1024)).toFixed(1)}MB /{' '}
                      {(systemInfo.performance.memory.totalJSHeapSize / (1024 * 1024)).toFixed(1)}MB
                    </span>
                  ...
                  <div className="text-sm">
                    <span className="text-bolt-elements-textSecondary">Utilisation du Heap: </span>
                    <span className="text-bolt-elements-textPrimary">
                      {systemInfo.performance.memory.usagePercentage.toFixed(1)}%
                    </span>
                  </div>
                  <div className="text-sm">
                    <span className="text-bolt-elements-textSecondary">Type de navigation: </span>
                    <span className="text-bolt-elements-textPrimary">
                  ...
                  <div className="text-sm">
                    <span className="text-bolt-elements-textSecondary">Redirections: </span>
                    <span className="text-bolt-elements-textPrimary">
        ...
        <CollapsibleTrigger className="w-full">
          <div className="flex items-center justify-between p-6 rounded-xl bg-white dark:bg-[#0A0A0A] border border-[#E5E5E5] dark:border-[#1A1A1A]">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-base font-medium text-bolt-elements-textPrimary">Informations de l'application web</h3>
          ...
          <div className="p-6 mt-2 rounded-xl bg-white dark:bg-[#0A0A0A] border border-[#E5E5E5] dark:border-[#1A1A1A]">
            {loading.webAppInfo ? (
              ...
              <div className="flex flex-col items-center justify-center p-8 text-bolt-elements-textSecondary">
                ...
                <p>Impossible de charger les informations de l'application web</p>
                <button
                  onClick={() => getWebAppInfo()}
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <h3 className="mb-4 text-base font-medium text-bolt-elements-textPrimary">Informations de base</h3>
                  <div className="space-y-3">
                    <div className="text-sm flex items-center gap-2">
                      <div className="i-ph:app-window text-bolt-elements-textSecondary w-4 h-4" />
                      <span className="text-bolt-elements-textSecondary">Nom:</span>
                    ...
                    <div className="text-sm flex items-center gap-2">
                      ...
                      <span className="text-bolt-elements-textSecondary">Licence:</span>
                    ...
                    <div className="text-sm flex items-center gap-2">
                      ...
                      <span className="text-bolt-elements-textSecondary">Environnement:</span>
                ...
                <div>
                  <h3 className="mb-4 text-base font-medium text-bolt-elements-textPrimary">Informations Git</h3>
                  <div className="space-y-3">
                    <div className="text-sm flex items-center gap-2">
                      <div className="i-ph:git-branch text-bolt-elements-textSecondary w-4 h-4" />
                      <span className="text-bolt-elements-textSecondary">Branche:</span>
                    ...
                    <div className="text-sm flex items-center gap-2">
                      ...
                      <span className="text-bolt-elements-textSecondary">Auteur:</span>
                      <span className="text-bolt-elements-textPrimary">{webAppInfo.gitInfo.local.author}</span>
                    </div>
                    <div className="text-sm flex items-center gap-2">
                      <div className="i-ph:clock text-bolt-elements-textSecondary w-4 h-4" />
                      <span className="text-bolt-elements-textSecondary">Temps du commit:</span>
                      ...
                      <>
                        <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-800">
                          <div className="text-sm flex items-center gap-2">
                            ...
                            <span className="text-bolt-elements-textSecondary">Dépôt:</span>
                            <span className="text-bolt-elements-textPrimary">
            ...
            {webAppInfo && (
              <div className="mt-6">
                <h3 className="mb-4 text-base font-medium text-bolt-elements-textPrimary">Dépendances</h3>
        ...
        <CollapsibleTrigger className="w-full">
          <div className="flex items-center justify-between p-6 rounded-xl bg-white dark:bg-[#0A0A0A] border border-[#E5E5E5] dark:border-[#1A1A1A]">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-base font-medium text-bolt-elements-textPrimary">Vérification des erreurs</h3>
              {errorLogs.length > 0 && (
                <Badge variant="destructive" className="ml-2">
                  {errorLogs.length} Erreurs
                </Badge>
              )}
            </div>
            <div
              ...
              <div className="space-y-4">
                <div className="text-sm text-bolt-elements-textSecondary">
                  ...
                  <ul className="list-disc list-inside mt-2 space-y-1">
                    <li>Erreurs JavaScript non gérées</li>
                    <li>Rejections de promesses non gérées</li>
                    <li>Exceptions de runtime</li>
                    <li>Erreurs de réseau</li>
                  </ul>
                </div>
                <div className="text-sm">
                  <span className="text-bolt-elements-textSecondary">Statut: </span>
                  <span className="text-bolt-elements-textPrimary">
                    {loading.errors
                      ? 'Vérification...'
                      : errorLogs.length > 0
                        ? `${errorLogs.length} erreurs trouvées`
                        : 'Aucune erreur trouvée'}
                  </span>
                </div>
                {errorLogs.length > 0 && (
                  <div className="mt-4">
                    <div className="text-sm font-medium text-bolt-elements-textPrimary mb-2">Erreurs récentes:</div>
                    <div className="space-y-2">
                      {errorLogs.map((error) => (
