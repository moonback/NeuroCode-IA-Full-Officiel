...
const ServiceStatusTab = () => {
  ...
  const checkApiEndpoint = useCallback(
    async (
      ...
      try {
        ...
        if (!response.ok) {
          let errorMessage = `L'API a renvoyé le statut : ${response.status}`;
        ...
        if (testModel && !models.includes(testModel)) {
          return {
            ...
            message: `Clé API valide (modèle ${testModel} non trouvé dans ${models.length} modèles disponibles)`,
            responseTime,
          };
        }
        ...
        return {
          ok: false,
          status: error instanceof Error ? error.message : 'Erreur inconnue',
      ...
      try {
        ...
        switch (provider) {
          ...
          default:
            return {
              status: 'operational',
              message: 'Vérification de la base uniquement',
              incidents: ['Remarque : informations limitées sur le statut en raison des restrictions CORS'],
            };
        }
      } catch (error) {
        ...
        return {
          status: 'degraded',
          message: 'Impossible de récupérer le statut en raison des restrictions CORS',
          incidents: ['Erreur : Impossible de vérifier le statut du service'],
        };
      }
    },
    [],
        ...
        try {
          ...
          if (!apiKey || !providerConfig) {
            return {
              ...
              icon: PROVIDER_ICONS[provider],
              message: !apiKey
                ? 'Status opérationnel (clé API nécessaire pour l\'utilisation)'
                : 'Status opérationnel (configuration nécessaire pour l\'utilisation)',
              incidents: [],
            };
          }
          ...
          return {
            ...
            icon: PROVIDER_ICONS[provider],
            message: ok ? 'Service et API opérationnels' : `Service opérationnel (API: ${message || status})`,
            responseTime,
            incidents: [],
          };
        } catch (error) {
          console.error(`Erreur lors de la récupération du statut pour ${provider} (tentative ${attempt}):`, error);
          ...
          return {
            ...
            statusUrl: config.statusUrl,
            icon: PROVIDER_ICONS[provider],
            message: 'Service opérationnel (Erreur de vérification du statut)',
            responseTime: 0,
            incidents: [],
          };
        }
  ...
  const fetchAllStatuses = useCallback(async () => {
    try {
      ...
      success('Statuts des API mis à jour avec succès');
    } catch (err) {
      console.error('Erreur lors de la mise à jour des statuts des API:', err);
      error('Échec de la mise à jour des statuts des API');
    } finally {
      setLoading(false);
    }
  ...
  const testApiKeyForProvider = useCallback(
    async (provider: ProviderName, apiKey: string) => {
      try {
        ...
        if (!config) {
          throw new Error('Configuration du fournisseur non trouvée');
        }
        ...
        switch (provider) {
          ...
          case 'Google': {
            ...
            if (result.ok) {
              ...
              success('La clé API est valide!');
            } else {
              setTestingStatus('error');
              error(`Échec du test de la clé API: ${result.message}`);
            }
        ...
        if (ok) {
          ...
          setTestingStatus('error');
          error(`Échec du test de la clé API: ${message}`);
        }
      } catch (err: unknown) {
        setTestingStatus('error');
        error('Échec du test de la clé API: ' + (err instanceof Error ? err.message : 'Erreur inconnue'));
      } finally {
      ...
      >
        <div className="flex items-center justify-between gap-2 mt-8 mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
            <div className="flex items-center gap-2">
                <h4 className="text-md font-medium text-bolt-elements-textPrimary">Statut des services API</h4>
                <span className="px-2 py-0.5 text-xs rounded-md bg-green-500/10 text-green-500 font-medium">
                  BETA
                </span>
              </div>
                            <p className="text-sm text-bolt-elements-textSecondary">
                Monitez et testez le statut opérationnel des fournisseurs de LLM cloud
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="p-4 bg-bolt-elements-background-depth-2 rounded-lg">
            <h5 className="text-sm font-medium text-bolt-elements-textPrimary mb-2">Test de la clé API</h5>
          <div className="flex gap-2">
            ...
            <input
              ...
              placeholder="Entrez la clé API à tester"
              className={classNames(
        ...
        {loading && serviceStatuses.length === 0 ? (
            <div className="text-center py-8 text-bolt-elements-textSecondary">Chargement des statuts des API...</div>
        ) : (
                      ...
                      <div>
                        ...
                        <div className="space-y-1">
                          <p className="text-xs text-bolt-elements-textSecondary">
                            Dernière vérification: {new Date(service.lastChecked).toLocaleTimeString()}
                          </p>
                          {service.responseTime && (
                            <p className="text-xs text-bolt-elements-textTertiary">
                              Temps de réponse: {Math.round(service.responseTime)}ms
                            </p>
                          )}
                          {service.message && (
...
ServiceStatusTab.tabMetadata = {
  ...
  description: 'Monitez et testez le statut opérationnel des fournisseurs de LLM cloud',
  category: 'services',
};
