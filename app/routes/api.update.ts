        ...
        try {
          ...
          try {
            ...
            if (files.length === 0) {
              sendProgress({
                stage: 'complete',
                message: `Aucun changement de fichier détecté entre votre version et upstream/${defaultBranch}. Vous êtes peut-être sur une branche différente.`,
                progress: 100,
                details: {
                  currentCommit: currentCommit.trim().substring(0, 7),
                  remoteCommit: remoteCommit.trim().substring(0, 7),
          ...
          sendProgress({
            ...
            details: {
              ...
              updateReady: true,
              changelog,
              compareUrl: `https://github.com/moonback/NeuroCode/compare/${currentCommit.trim().substring(0, 7)}...${remoteCommit.trim().substring(0, 7)}`,
            },
          });
